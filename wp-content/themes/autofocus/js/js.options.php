...
<?php }
	...
	if ( ( is_home() && $home_layout == 'grid') || ( $archive_layout == 'images' && $home_layout == 'grid' && ( is_archive() || is_search() ) ) ) {
		...
		<?php } elseif ( $title_date == 'title' ) { ?>
			...
			// Hover animations for front page
			/* 
			* krishnas: remove hover color in stick post slider image by replacing '.post' with '.hentry'
			* the hover is disabled only when wordpress theme option is set as 'Post Title Only' and layout as 'Grid Layout'
			* to get there, login to wordpress admin portal; Left Panel:- Appearance -> Theme Options -> General Options -> Post Title Only
			* help link: http://getsatisfaction.com/fthrwght/topics/af_2_0_sticky_post_slider_how_to_remove_hover_colour_and_text
			*/
